...
**Domain:** Mobile-first event-staffing web app (internal hiring dashboard + public no-auth magic-link accept/decline pages) on an existing Supabase (HITO) data layer, zero-budget
**Researched:** 2026-07-10
**Confidence:** HIGH

## TL;DR Decision

**Build a standalone Next.js app that mirrors the HITO stack exactly.** Not Astro, not a module inside the HITO repo.
...
Do **not** use Supabase Auth's built-in magic link for the staff accept/decline flow — that would create auth users, which v1 explicitly avoids. Instead replicate HITO's public-token pattern:
...
Just build the URL — no dependency:
`https://wa.me/<E164_number_no_plus_no_spaces>?text=<encodeURIComponent(message)>`. Store staff phone in E.164, strip `+`/spaces at link-build time. One-tap from Franco's phone; keeps you off the paid WhatsApp Business API (explicit out-of-scope).
...
**Use `@supabase/supabase-js` as the primary runtime data layer. Use Drizzle only for migrations/schema/types, if at all.**

Rationale — this is a correctness/security decision, not a style one:
...
npx create-next-app@15 staff-app --typescript --app --tailwind --eslint

# Core data + auth
npm install @supabase/supabase-js@^2.110 @supabase/ssr@^0.12

# UI (HITO parity — lets you copy its components)
npm install @base-ui/react@^1.6 lucide-react class-variance-authority clsx tailwind-merge sonner next-themes motion

# Forms + validation
npm install react-hook-form@^7.81 zod@^4.4 @hookform/resolvers@^5.4

# Email (own SMTP, zero cost)
npm install nodemailer@^9 @react-email/components@^1 @react-email/render@^1

# Dates
npm install date-fns@^4

# Dev
npm install -D @tailwindcss/postcss autoprefixer typescript @types/node @types/nodemailer
...
**If Franco wants passwordless admin login:**
...
**If email deliverability from Ferozo SMTP is shaky:**
...
---
*Stack research for: mobile-first event-staffing app on existing Supabase (HITO), zero-budget*
*Researched: 2026-07-10*
