...
Last activity: 2026-07-31 - Quick task 260731-s6w: logo oficial, escala de titulos y separacion super admin / productora (migracion 0044 aplicada y verificada). Antes: 260731-q8a y 260731-pe7
...
|-------|------|--------------|--------|
| 2026-08-05 | [260803-prv-alta-abierta-de-proveedores](quick/260803-prv-alta-abierta-de-proveedores/260803-prv-PLAN.md) | El proveedor se anota SOLO: era lo unico que faltaba del camino del proveedor. Todo el resto ya existia desde la 0042 (perfil, servicios, formulario, publicarse y despublicarse por token), pero el unico generador de tokens era `staff_app_generar_link_proveedor`, que exige ser writer de una productora y que **ningun archivo del repo llamaba**: nadie podia empezar. Migracion `0060` con `staff_app_registrar_proveedor` (perfil COMPLETO + servicios en un solo paso, `is_public=true` al toque, token crudo devuelto una vez, solo `service_role`), ruta publica `/registrar-proveedor` con formulario completo, mail de bienvenida que NO promete cuenta ni contrasena, y las dos puertas: tercera tarjeta en la landing ("Dos caminos" paso a "Tres caminos") y CTA en `/servicios`. **Agujero encontrado y tapado en el camino: `staff_app_plataforma_moderar` solo modera `gig_openings`, asi que Franco NO podia bajar a un proveedor publicado** y el aviso del alta hubiera sido un aviso sin boton, con el antecedente de la obscenidad en la bio. Migracion `0061` (`staff_app_plataforma_proveedores` + `staff_app_plataforma_moderar_proveedor`) y pantalla `/plataforma/proveedores`. **0060 y 0061 APLICADAS y verificadas llamando las funciones reales contra produccion**: las 6 validaciones, la normalizacion de provincias, la reinscripcion que no pisa datos, permisos (anon y authenticated NO pueden ejecutar el alta), el gate `is_platform_admin` fallando cerrado, y el ciclo vidriera 1 -> bajar -> 0 -> resubir -> 1. | complete ✓ (falta el human-check: que Franco se anote como proveedor de prueba y le llegue el mail) |
| 2026-08-03 | [260803-idq-identidad-al-salir-y-contrasena-en-login](quick/260803-idq-identidad-al-salir-y-contrasena-en-login/260803-idq-SUMMARY.md) | Una productora se registraba, elegia su contrasena y la app le decia "esta cuenta no tiene acceso". `definir-contrasena` salia a `/panel-staff` escrito a mano (pantalla que comparten staff y productora), asi que se salteaba el UNICO lugar donde corre `staff_app_provision_member` y la org quedaba con CERO miembros; `login/page.tsx` mandaba a `/dashboard` sin mirar identidad, asi que un empleado logueado veia la misma pantalla de rechazo. Los dos salen ahora por `/auth/callback`. Ademas `/login` no tenia campo de contrasena (0 menciones) mientras el mail prometia "despues entras siempre con este mismo mail y esa clave": `signInWithPassword` se mudo a `lib/auth-password.ts` y la usan las dos puertas. **La regla: toda pantalla compartida por dos actores termina en `/auth/callback`, nunca en un panel escrito a mano** (tercera vez que aparece el patron). La funcion NO estaba rota: se la llamo simulando la sesion y devolvia `owner` correcto. Commit `04d565e`. | complete ✓ (falta el human-check: que Franco entre y somos-der-2 pase de 0 a 1 miembro) |
...
| 2026-07-31 | [260731-pe7-movimiento-2-del-marketplace-puerta-de-e](quick/260731-pe7-movimiento-2-del-marketplace-puerta-de-e/260731-pe7-SUMMARY.md) | Marketplace de 3 lados, movimiento 2: la puerta de entrada del proveedor. El movimiento 1 dejó el perfil creado pero al proveedor sin forma de entrar. Migración `0042` (token hasheado en `marketplace_profiles` + 6 RPCs `SECURITY DEFINER`) y la ruta pública `/acceso-proveedor/[token]`: entra sin cuenta, completa su perfil, carga sus servicios con zonas y se publica solo. `is_verified`, `slug` y `rating` quedan fuera de su alcance a propósito. Commits `b8419e9`, `749b8dc`, `b5cc8bd`. **`0042` y `0043` APLICADAS y verificadas: harness 35/35, advisor sin findings nuevos.** ⚠️ El harness encontró un bug real que había pasado typecheck, lint y build: `v_faltan := v_faltan || 'texto'` sobre un `text[]` revienta con 22P02, así que publicarse sin servicios tiraba excepción en vez de decir qué falta. Arreglado en la `0043` (`array_append`), commit `bb9f6b3`. | complete ✓ |
| 2026-07-31 | [260731-s6w-logo-oficial-escala-de-titulos-y-separar](quick/260731-s6w-logo-oficial-escala-de-titulos-y-separar/260731-s6w-SUMMARY.md) | Franco entró al portal como productora y no reconoció el producto ("estoy viendo mi perfil de super administrador"). Tres frentes: (1) el wordmark oficial reemplaza **16 copias escritas a mano** en 13 archivos, con un componente único, y se borra el `@utility font-lockup` que queda sin consumidores (era una regla dura de Franco que no se cumplía); (2) escala tipográfica real (`t-display`/`t-stat`/`t-stat-sm`/`t-section`) sobre 26 archivos, se tira el parche de `!important`, y desaparecen los títulos de 120px que en un teléfono de 390px eran absurdos; (3) migración `0044`: `organizations.es_plataforma` separa a la org DUEÑA del producto de una productora cliente, con gate **en el servidor** (`lib/org.ts`), así `/leads` deja de aparecerle a cualquier productora. Backfill por `is_default`, sin volver a hardcodear el UUID que la 0035 sacó de 31 funciones. Commits `69f0bde`, `3b44072`, `546f24a`, `f9e573f`. **0044 aplicada: harness 15/15, incluido que `authenticated` no tiene UPDATE sobre `organizations` (nadie se auto-asciende a plataforma con un PATCH). Advisor sin cambios.** | complete ✓ |
