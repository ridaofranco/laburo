...
### Infraestructura (decisión de Franco, 2026-07-14 — SUPERSEDES "proyecto Supabase nuevo")
- **D-03:** La Staff App vive en un **schema propio `staff_app` DENTRO del proyecto Supabase de HITO** (`luillpzfqzbpoqkgvjuw`), NO en un proyecto nuevo. Razón: el tier gratis permite 2 proyectos activos y ambos están en uso (somosder-sales y HITO); Franco no quiere pausar ninguno ni pagar, y eligió HITO (no sales) porque "va a estar enlazado a HITO al final del proyecto". La independencia del producto es LÓGICA, no física: tablas propias en `staff_app.*`, RLS propia, org-scoping propio; NADA se escribe en las tablas `public.*` de HITO en esta fase (siguen siendo intocables hasta la Fase 6). Consecuencias: (a) el form web conserva la MISMA URL/anon key — el corte es solo apuntar el insert al nuevo destino; (b) los CVs ya viven en el bucket de este proyecto — no hay migración de archivos; (c) el backfill Source A es un INSERT...SELECT dentro de la misma base; (d) el puente de la Fase 6 se simplifica (misma base). Si algún día hay slot/presupuesto, el schema es migrable a proyecto propio.
