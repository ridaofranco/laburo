...
```
- The service-role client can write to `staff_app.members` server-side (`.schema('staff_app')` sets the request profile; service-role bypasses PostgREST schema exposure for its own key **only server-side**). Alternative: a `SECURITY DEFINER` `public.staff_app_provision_member()` that self-provisions if `auth.email()` ∈ allowlist (keeps the allowlist in the DB). Either works — the server-action/service-role variant keeps the allowlist in code and needs no new DB function.
